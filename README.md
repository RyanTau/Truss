...
## Supported commands in 0.1.7

- Turn **lights, switches, fans, and input booleans** on/off.
- Activate **scenes and scripts**.
- Say or type a device name or Assist alias and an operation, such as **tall lamp on please** or **could you put the Turkish lamp off**. Minor name misspellings and unique shortened names of at least two words are accepted. Give `light.ball_lamp` the alias **Turkish lamp** if that is what you call it. Similar or duplicate matches still wait; room-only requests and arbitrary paraphrases are not supported.
...
The engine first resolves a unique device name/alias, allowing close spelling matches, and a supported operation from each partial transcript. Laya then chooses between that concrete action and `wait` in a single decision. This replaces the old separately scored groups, whose probabilities could unfairly favour an unrelated device. There is no comparison or renormalization across groups. All candidate IDs remain in the event: ineligible actions have zero; the eligible action receives Laya's unmodified probability conditional on the resolved device/operation. `score_scope: resolved_action` identifies these semantics. These are not whole-home probabilities or calibrated correctness guarantees. New installations default to 0.80; existing configured thresholds are preserved. The resolver requires an on/off operation (or scene/script activation) and waits on missing targets, multiple targets, conflicting operations, or recognized negation. It cannot anticipate a later spoken correction.

Bundled transcription uses streaming beam search with per-session device-name and alias hints. Close spelling matches are allowed, but strongly garbled names such as “colored lion” still wait. Assist aliases remain useful for nicknames that do not resemble the friendly name.

### Updating to 0.1.7

Version 0.1.7 adds typed commands, close name matching, shorter names, and a more permissive Laya instruction. New installations default to a **0.80** probability threshold. Existing settings are preserved: for more forgiving commands, open Truss **Configure** and change **Minimum action probability** from 0.95 to **0.80**. Lower thresholds accept more uncertain interpretations. Name matching requires every word in the matched phrase to be close and a clear lead over other devices; it does not infer unrelated nicknames. Restart both the updated engine and integration. No new dependencies were added after 0.1.6.

Update **both** the HACS integration and the separate engine, then restart each. HACS does not update the engine. On Windows, stop the engine, run `git pull` in the Truss repository, and run `powershell -NoProfile -File scripts/run_engine_windows.ps1 -Install` to install the new `sentencepiece` dependency and start it. If using your own Cygwin launcher, install `truss_engine/requirements.txt` into its existing Python environment and keep using its existing token/cache configuration. For Docker, run `git pull` followed by `docker compose up -d --build`. The first updated engine start downloads a pinned 245 KB speech tokenizer; subsequent runs reuse the cache. Authenticated `/health` reports `engine_version: 0.1.7` and `score_scope: resolved_action`.
